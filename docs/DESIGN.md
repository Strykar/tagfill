...
**`census.py` decides the file universe once.** Always a full unscoped walk, even when `--path` is given: `census.csv` is shared ground truth, and scoping it there would silently truncate the file every other stage reads. `--path` restricts what *consuming* stages act on. A census is stale the moment a stage writes, so `census.load()` folds the journal's later applies back over the rows before handing them out — otherwise a second `mb --recheck` re-queries MusicBrainz for folders whose only gap was already filled, and `--recheck` is exactly the flag that turns off the resume guard that would have hidden it. Writes older than the census lose to it; that reading is the more recent one.

**`probe.py` is the container chokepoint.** Explicit suffix → (mutagen class, family) dispatch. Never `easy=True`: it has no Easy wrapper for AIFF or WAVE, silently returns the raw object, and a fully tagged file then reads as untagged. `probe.FIELDS` is the complete set tagfill will ever write, which is what makes the restore guarantee scopeable.
...
**A write preserves the ID3 version it found.** mutagen's `save()` writes ID3v2.4 whatever it read, so filling one blank genre used to upgrade the tag of every mp3 it touched, and v2.3 is the version Windows Explorer and older hardware players read reliably. `probe._atomic_save()` reads the existing version and passes it back to `save()`. It preserves; it never downgrades a file that was already v2.4.
...
- **The submit stage has never run against the live AcoustID endpoint.** Selection, the worklist, the three refusal paths and both journal outcomes are covered with the POST mocked, but no real submission has been made.
